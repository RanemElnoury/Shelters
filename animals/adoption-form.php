...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == "POST") {
  ...
  $animal_id= filter_var($_POST['animal_id'], FILTER_SANITIZE_STRING); 
 ...
 if(in_array($personal_image_extention,$extention_allowed)){
  ...
  $sql = "INSERT INTO animal_adopt (f_name, l_name, email, ID_Number,phone,userimage,animal_id)  VALUES(?,?,?,?,?,?,?)";

  
  try {
    global $conn;
    $stmt = $conn->prepare($sql);
    $stmt->execute(array($f_name,$l_name,$email,$ID_Number,$phone,$final_personal_image_name,$animal_id));
    ?>
    <script>
...
<div class="col-lg-4 div-form position-absolute top-50 end-0 translate-middle-y">
 <form class="card w-100" method="POST" enctype="multipart/form-data">
  <div class="justify-content-center flex-row d-flex mt-5">
   <img src="uploads/OIP.jpeg" id="profile-pic">
  </div>
  ...
  <?php
    ...
    if (isset($_GET['id'])) {
        $stmt = $conn->prepare('SELECT id FROM animals_data WHERE id=?');
        $stmt->execute(array($_GET['id']));
        $kids_data = $stmt->fetchAll();    
?>
    <?php foreach($kids_data as $data){ ?>
    <input type="hidden" name="animal_id" value="<?php echo $data['id']?>">
<?php
    }
  }
    }
