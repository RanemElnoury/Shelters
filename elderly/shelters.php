...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'GET') {
    if (isset($_GET['id'])) {
        $stmt = $conn->prepare('SELECT * FROM elderly_shelters WHERE id=?');
...
<h2 style="text-align:center;"class="my-4"><?php echo $data['name']?></h2>
 ...
 <div class="justify-content-center flex-row d-flex">
   <a href="elderly.php?id=<?php echo $data['id']?>"  class="btn btn-light py-2 px-5 text-uppercase justify-content-center mb-3" style="align-item:center;">want to join us ?!</a>
 </div>
</div>
<?php
}
