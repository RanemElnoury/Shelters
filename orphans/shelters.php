...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'GET') {
    if (isset($_GET['id'])) {
        $stmt = $conn->prepare('SELECT * FROM orphan_shelters WHERE id=?');
...
<h2 style="text-align:center;"class="my-4"><?php echo $data['name']?></h2>
 ...
 <div class="justify-content-center flex-row d-flex">
   <a href="orphans.php?id=<?php echo $data['id']?>"  class="btn btn-light py-2 px-5 text-uppercase justify-content-center mb-3" style="align-item:center;">want to see our children ?!</a>
 </div>
</div>
<?php
}
